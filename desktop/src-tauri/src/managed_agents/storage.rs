...
#[cfg(windows)]
#[path = "file_permissions.rs"]
mod file_permissions;
#[cfg(windows)]
use file_permissions::restrict_file_to_current_user;
...
/// Atomic, symlink-preserving JSON write that restricts the destination to the
/// account that owns it. Used for `managed-agents.json`, which carries
/// plaintext agent nsecs in the keyringless fallback. Mirrors
/// [`crate::app_state::save_key_file`].
///
/// **The restriction is not the same on every platform, and the difference is
/// load-bearing** — an earlier version of this comment claimed `0o600`
/// "unconditionally", which was false on Windows and left plaintext private
/// keys readable by every account the parent directory's ACL admitted:
///
/// * **Unix** — `chmod 0o600` is applied to the atomic-write handle *before*
///   any secret bytes reach the filesystem, closing the umask window a
///   post-write `chmod` would leave open. A failure here aborts the write.
/// * **Windows** — there is no mode to set. After the atomic rename the
///   committed file is given an explicit *protected* (inheritance-blocked)
///   DACL naming only the current user; see
///   [`restrict_file_to_current_user`]. Without it the file simply inherits
///   the parent directory's ACL, which on real installs has granted Modify to
///   non-owner groups.
/// * **Any other platform** — no restriction is applied at all.
///
...
pub(crate) fn atomic_write_json_restricted(path: &Path, payload: &[u8]) -> Result<(), String> {
    ...
    #[cfg(windows)]
    {
        // Why after the commit and not before the bytes: `atomic-write-file`
        // writes into a randomly-named temporary in the same directory and
        // *renames* it into place, so the DACL that survives is the
        // temporary's. The crate exposes neither that path nor a handle opened
        // with `WRITE_DAC`, so the destination can only be tightened once the
        // rename has happened. A short window therefore remains in which the
        // temporary carries the directory's inherited ACL — strictly better
        // than the permanent exposure this replaces, and not fixable without
        // hand-rolling the atomic write (which would put the user's whole
        // agent list at risk for a millisecond-scale race).
        //
        // A failure here is loud but NOT fatal: every byte is already
        // committed, so returning `Err` would report a save that succeeded as
        // failed, and callers are entitled to act on that (retry, surface an
        // error, unwind an import). Losing the agent list over a failed
        // permission tweak is the worse outcome, so the error is reported at
        // `error!` — never swallowed — and the write stands.
        if let Err(error) = restrict_file_to_current_user(&resolved) {
            tracing::error!(
                event = "managed_agents_restrict_acl_failed",
                path = %resolved.display(),
                error = %error,
                "could not restrict managed-agents.json to its owner; it may be \
                 readable by other accounts on this machine"
            );
        }
    }
...
/// Replace `path`'s DACL with a single access-allowed entry granting the
/// current user full control, and mark that DACL *protected* so the parent
/// directory's (typically far more permissive) inheritable ACEs no longer
/// apply.
///
/// Implementation notes:
///
/// * `SetFileSecurityW` is used rather than the more modern
///   `SetNamedSecurityInfoW` because it lives in `windows-sys`'s
///   `Win32_Security` feature, which this crate already enables for the named
///   mutex in `secret_store.rs`. `SetNamedSecurityInfoW` would require adding
///   `Win32_Security_Authorization`, and `SetEntriesInAclW` with it, for no
///   behavioural gain here.
/// * Inheritance is blocked two ways — the `SE_DACL_PROTECTED` control bit on
///   the descriptor itself, and the `PROTECTED_DACL_SECURITY_INFORMATION`
///   request flag. The legacy entry point is documented against the control
///   bit and is not guaranteed to honour the request flag, so the call is
///   retried without the flag if it is rejected outright.
/// * Every failure returns `Err` with the `GetLastError` text. Nothing here is
///   swallowed; the caller logs it at `error!`.
...
/// Regression cover for [`atomic_write_json_restricted`], which writes the file
/// that holds plaintext agent nsecs when there is no OS keyring.
#[cfg(test)]
